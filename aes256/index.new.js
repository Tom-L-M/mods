...
function encrypt(_text, _password, _ivkeyword) {
    const key = sha256(_password).slice(0, 16);
    const iv = sha256(_ivkeyword).slice(0, 8);
...
function printVersion() {
    try {
        console.log(require(exeresolve('{{{}}}/package.json')).version);
    } catch (err) {
...
(async function Main() {
    ...
    Usage: 
        node aes256 <filename> [Options]
       OR
        <input> | node aes256 [options]

    Options:
        -h | --help         Prints the help message and quits.
        -v | --version      Prints the version info and quits.
        -d | --decrypt      Decrypts data instead of encrypting.

    Info:
        > After the first arguments are provided, the tools enters interactive mode.
        > About encryption method and password:
            > The password you provide for encryption, is hashed with sha256,
              and then the first 16 bytes are used as key (first 128 bits).
            > The IV keyword you provide for encryption, is hashed with sha256, 
              and then the first 8 bytes are used as IV (first 64 bits).
